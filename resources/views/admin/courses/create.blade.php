...
@section('header', 'নতুন কোর্স অ্যাড করুন')
...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
    <form action="{{ route('admin.courses.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-8" x-data="courseForm()">
        @csrf
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">কোর্সের শিরোনাম <span class="text-red-500">*</span></label>
                    <input type="text" name="title" value="{{ old('title') }}" required 
                           class="block w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="উদা: সম্পূর্ণ ওয়েব ডেভেলপমেন্ট গাইড">
                ...
                <!-- Category -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ক্যাটাগরি <span class="text-red-500">*</span></label>
                ...
                <!-- Level -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">লেভেল <span class="text-red-500">*</span></label>
                    <select name="level" required class="block w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                ...
                <!-- Language -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ভাষা <span class="text-red-500">*</span></label>
                    <select name="language" required class="block w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @foreach($languages as $lang)
                            <option value="{{ $lang }}" {{ old('language', 'bengali') == $lang ? 'selected' : '' }}>{{ ucfirst($lang) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Price -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">মূল্য (৳) <span class="text-red-500">*</span></label>
                ...
                <div class="col-span-2 md:col-span-1">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">কোর্স থাম্বনেইল</label>
                    ...
                    <p class="mt-1 text-xs text-gray-500">প্রস্তাবিত সাইজ: 1280x720px (JPG, PNG)</p>
                </div>

                <!-- Video Preview -->
                <div class="col-span-2 md:col-span-1">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">প্রিভিউ ভিডিও (ঐচ্ছিক)</label>
        ...
        <div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="short_description" rows="2" class="block w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('short_description') }}</textarea>
                </div>
                <div>
                    ...
                    <textarea name="description" rows="6" required class="block w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <!-- Requirements -->
            <div class="bg-gray-50 dark:bg-slate-700/30 p-4 rounded-lg border border-gray-200 dark:border-slate-700">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">পূর্বশর্ত (Requirements)</label>
        ...
        <div class="bg-blue-50 dark:bg-blue-900/10 p-5 rounded-lg border border-blue-100 dark:border-blue-800/30">
            <div class="flex flex-wrap gap-6">
                <label class="inline-flex items-center cursor-pointer">
                    ...
                    <input type="checkbox" name="featured" value="1" class="form-checkbox h-5 w-5 text-blue-600 rounded border-gray-300 bg-white dark:bg-slate-700 dark:border-slate-600 focus:ring-blue-500">
                ...
                <label class="inline-flex items-center cursor-pointer">
                    ...
                    <input type="checkbox" name="certificate_included" value="1" checked class="form-checkbox h-5 w-5 text-blue-600 rounded border-gray-300 bg-white dark:bg-slate-700 dark:border-slate-600 focus:ring-blue-500">
                ...
                <label class="inline-flex items-center cursor-pointer">
                    ...
                    <input type="checkbox" name="lifetime_access" value="1" checked class="form-checkbox h-5 w-5 text-blue-600 rounded border-gray-300 bg-white dark:bg-slate-700 dark:border-slate-600 focus:ring-blue-500">
                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">লাইফটাইম এক্সেস?</span>
                </label>
            </div>

            <div class="mt-6 max-w-xs">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">স্ট্যাটাস <span class="text-red-500">*</span></label>
                <select name="status" required class="block w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:ring-blue-500">
                    @foreach($statuses as $status)
                        <option value="{{ $status }}" {{ $status == 'draft' ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $status)) }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="flex justify-end pt-4">
            <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                <i class="fas fa-save mr-2"></i> কোর্স তৈরি করুন
            </button>
        </div>
    </form>
</div>
