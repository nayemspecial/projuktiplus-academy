...
@section('title', 'নতুন প্রশ্ন তৈরি করুন - ProjuktiPlus LMS Admin')
...
<div class="mx-auto">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg overflow-hidden border border-gray-200 dark:border-slate-700">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-700 dark:to-slate-800">
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white">নতুন প্রশ্ন তৈরি করুন</h3>
        </div>
            ...
            <div class="px-6 py-6 space-y-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="quiz_id" id="quiz_id" 
                            ...
                            @foreach($quizzes as $id => $title)
                                <option value="{{ $id }}" {{ old('quiz_id') == $id ? 'selected' : '' }}>{{ $title }}</option>
                            @endforeach
                        </select>
                        @error('quiz_id')
                ...
                <div>
                    ...
                    <textarea name="explanation" id="explanation" rows="3" 
                              class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-700 dark:text-white">{{ old('explanation') }}</textarea>
                    @error('explanation')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
