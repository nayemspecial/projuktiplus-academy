...
@section('title', 'সকল উত্তর - ProjuktiPlus LMS Admin')
...
<div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg overflow-hidden border border-gray-200 dark:border-slate-700">
    ...
    <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-700 dark:to-slate-800">
        <div class="flex flex-col md:flex-row justify-between items-center gap-3">
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white">সকল উত্তর</h3>
            <div class="relative">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            <thead class="bg-gray-50 dark:bg-slate-700">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">উত্তর</th>
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <div class="text-sm font-medium text-gray-900 dark:text-white line-clamp-2">{{ Str::limit($answer->answer, 70) }}</div>
                            </div>
                        </div>
                    </td>
                    ...
                    <!-- প্রশ্ন -->
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-900 dark:text-white">{{ Str::limit($answer->question->question, 50) }}</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $answer->question->type }}</div>
                    </td>
                    
                    <!-- কুইজ -->
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-900 dark:text-white">{{ $answer->question->quiz->title }}</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $answer->question->quiz->lesson->section->course->title }}</div>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex justify-end gap-3">
                            ...
                            <form action="{{ route('admin.answers.destroy', $answer) }}" method="POST" class="inline-block" 
                                  onsubmit="return confirm('আপনি কি নিশ্চিত এই উত্তর ডিলিট করতে চান?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
