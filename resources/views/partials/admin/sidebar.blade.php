<aside class="hidden lg:flex flex-col w-64 h-full fixed inset-y-0 left-0 z-30 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 transition-all duration-300">
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
        ...
        <div x-data="{ open: {{ request()->routeIs('admin.users.*') ? 'true' : 'false' }} }">
            <button @click="open = !open" class="flex items-center justify-between w-full px-4 py-3 text-sm font-medium rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-slate-700 {{ request()->routeIs('admin.users.*') ? 'text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300' }}">
                <div class="flex items-center">
                    ...
                    <span class="ml-3">সকল ইউজার</span>
                </div>
        ...
        <div x-data="{ open: {{ request()->routeIs('admin.coupons.*') || request()->routeIs('admin.email-templates.*') ? 'true' : 'false' }} }">
            <button @click="open = !open" class="flex items-center justify-between w-full px-4 py-3 text-sm font-medium rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-slate-700 {{ request()->routeIs('admin.coupons.*') || request()->routeIs('admin.email-templates.*') ? 'text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300' }}">
                <div class="flex items-center">
                    <i class="fas fa-bullhorn w-5 text-center"></i>
                    <span class="ml-3">মার্কেটিং ও প্রমোশন</span>
                </div>
                <i class="fas fa-chevron-down text-xs transition-transform duration-200" :class="{'rotate-180': open}"></i>
            </button>
            <div x-show="open" x-collapse class="pl-11 space-y-1 mt-1">
                <a href="{{ route('admin.coupons.index') }}" class="block px-4 py-2 text-sm rounded-lg {{ request()->routeIs('admin.coupons.*') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-700/50' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200' }}">কুপন ম্যানেজমেন্ট</a>
                
                <a href="{{ route('admin.email-templates.index') }}" class="block px-4 py-2 text-sm rounded-lg {{ request()->routeIs('admin.email-templates.*') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-700/50' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200' }}">ইমেইল টেমপ্লেট</a>
            </div>
        </div>
