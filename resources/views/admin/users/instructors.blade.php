...
@section('title', 'সকল ইনস্ট্রাক্টর')

@section('header', 'ইনস্ট্রাক্টরদের তালিকা')
...
@section('actions')
    <div class="flex gap-2">
        ...
        <a href="{{ route('admin.users.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none transition-colors">
            <i class="fas fa-user-plus mr-2"></i> নতুন ইনস্ট্রাক্টর
        </a>
    </div>
@endsection
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-gray-50 dark:bg-slate-700/50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">তৈরিকৃত কোর্স</th>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <img class="h-10 w-10 rounded-full object-cover ring-2 ring-purple-100 dark:ring-purple-900" 
                                         src="{{ $instructor->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode($instructor->name).'&background=random' }}" 
                                         alt="{{ $instructor->name }}">
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $instructor->name }}</div>
                                    <div class="text-xs text-gray-400">ID: {{ $instructor->id }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300">
                                <i class="fas fa-chalkboard-teacher mr-1.5"></i> {{ $instructor->courses->count() }} টি
                            </span>
                        </td>

                        <!-- Status -->
                        <td class="px-6 py-4 whitespace-nowrap">
                            <form action="{{ route('admin.users.status', $instructor->id) }}" method="POST" class="flex items-center">
                                @csrf
                                <input type="hidden" name="status" value="{{ $instructor->status === 'active' ? 'inactive' : 'active' }}">
                                <button type="submit" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 {{ $instructor->status === 'active' ? 'bg-green-500' : 'bg-gray-200 dark:bg-slate-600' }}" role="switch" aria-checked="{{ $instructor->status === 'active' }}">
                                    <span class="sr-only">স্ট্যাটাস পরিবর্তন</span>
                                    <span aria-hidden="true" class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ $instructor->status === 'active' ? 'translate-x-4' : 'translate-x-0' }}"></span>
                                </button>
                            </form>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end items-center gap-2">
                                ...
                                <form action="{{ route('admin.users.destroy', $instructor->id) }}" method="POST" onsubmit="return confirm('আপনি কি নিশ্চিত যে এই ইনস্ট্রাক্টরকে ডিলিট করতে চান?');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-gray-500 dark:text-gray-400 font-medium">কোনো ইনস্ট্রাক্টর পাওয়া যায়নি</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
