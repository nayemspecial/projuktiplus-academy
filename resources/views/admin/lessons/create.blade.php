...
@section('title', 'নতুন লেসন তৈরি করুন - ProjuktiPlus LMS Admin')
...
<div class="mx-auto">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700">
            <h3 class="text-lg font-medium text-gray-800 dark:text-white">নতুন লেসন তৈরি করুন</h3>
        </div>
            ...
            <div class="px-6 py-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="section_id" id="section_id" 
                            ...
                            @foreach($sections as $id => $title)
                                <option value="{{ $id }}" {{ old('section_id') == $id ? 'selected' : '' }}>{{ $title }}</option>
                            @endforeach
                        </select>
                        @error('section_id')
                    ...
                    <div>
                        ...
                        <select name="video_type" id="video_type" 
                            ...
                            @foreach($videoTypes as $value => $label)
                                <option value="{{ $value }}" {{ old('video_type') == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('video_type')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="video_duration" id="video_duration" value="{{ old('video_duration') }}" min="0" 
                               class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-700 dark:text-white">
                    ...
                    <div>
                        <label class="flex items-center mt-6">
                            <input type="checkbox" name="preview" value="1" {{ old('preview') ? 'checked' : '' }} 
                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-slate-600 rounded dark:bg-slate-700">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="6" 
                              class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-700 dark:text-white">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
